...
import (
	...
	"github.com/axiacoin/axia-network-v2-magellan/replay"
	oreliusRpc "github.com/axiacoin/axia-network-v2-magellan/rpc"
	"github.com/axiacoin/axia-network-v2-magellan/services/rewards"
...
func producerFactories(sc *servicesctrl.Control, cfg *cfg.Config) []utils.ListenCloser {
	...
	factories = append(factories, stream.NewProducerAXCChain(sc, *cfg))
	for _, v := range cfg.Chains {
		switch v.VMType {
		case consumers.IndexerAVMName:
			p, err := stream.NewProducerChain(sc, *cfg, v.ID, stream.EventTypeDecisions, stream.IndexTypeTransactions, stream.IndexSwapChain)
			if err != nil {
				panic(err)
			}
			factories = append(factories, p)
			p, err = stream.NewProducerChain(sc, *cfg, v.ID, stream.EventTypeConsensus, stream.IndexTypeVertices, stream.IndexSwapChain)
			if err != nil {
				panic(err)
			}
			factories = append(factories, p)
		case consumers.IndexerPVMName:
			p, err := stream.NewProducerChain(sc, *cfg, v.ID, stream.EventTypeDecisions, stream.IndexTypeBlocks, stream.IndexCoreChain)
			if err != nil {
				panic(err)
			}
			factories = append(factories, p)
	...
	if sc.IsAXCChainIndex {
		p, err := stream.NewProducerChain(sc, *cfg, cfg.AXCchainID, stream.EventTypeDecisions, stream.IndexTypeBlocks, stream.IndexAXCChain)
		if err != nil {
			panic(err)
		}
		factories = append(factories, p)
