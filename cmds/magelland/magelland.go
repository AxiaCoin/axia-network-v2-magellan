...
func producerFactories(sc *servicesctrl.Control, cfg *cfg.Config) []utils.ListenCloser {
	...
	factories = append(factories, stream.NewProducerAXChain(sc, *cfg))
	...
	if sc.IsAXChainIndex {
		p, err := stream.NewProducerChain(sc, *cfg, cfg.AXchainID, stream.EventTypeDecisions, stream.IndexTypeBlocks, stream.IndexAXChain)
		if err != nil {
			panic(err)
		}
		factories = append(factories, p)
